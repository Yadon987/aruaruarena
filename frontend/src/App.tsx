...
import { JudgeAvatar } from "./features/judging/components/JudgeAvatar";
...
function validateForm(nickname: string, body: string): ValidationErrors {
	...
	return {
		...
		bodyError:
			trimmedBody.length >= MIN_BODY_LENGTH ? "" : MESSAGE_BODY_REQUIRED,
	};
}
...
function resolveSubmitErrorMessage(error: unknown): string {
	...
	if (
		error instanceof ApiClientError &&
		SERVER_ERROR_STATUSES.includes(error.status)
	) {
						...
						return (
							<li key={item.id}>
								...
								>
									...
									{isMyPost && (
										<p className="text-sm font-bold">あなたの投稿</p>
									)}
								</button>
							</li>
						);
...
function App() {
	...
	const [judgingNickname, setJudgingNickname] = useState(
		MESSAGE_JUDGING_NICKNAME_FALLBACK,
	);
	...
	useEffect(() => {
		...
		const fetchPost = async () => {
			...
			try {
				...
				setJudgingNickname(
					response.nickname || MESSAGE_JUDGING_NICKNAME_FALLBACK,
				);
							...
							{JUDGE.PERSONAS.map((persona) => (
								<li key={persona}>
									<JudgeAvatar
										...
										className="h-16 w-16"
									/>
									<p>{JUDGE_LABELS[persona]}</p>
									{persona === "hiroyuki" && (
										<p data-testid="catchphrase-hiroyuki">
											{HIROYUKI_CATCHPHRASE}
										</p>
									)}
								</li>
						...
						<div className="w-full max-w-md rounded bg-white p-4">
							{selectedPost ? (
								...
								<>
									...
									{isLoadingPostDetail && (
										<p className="mb-3">投稿詳細を読み込み中です...</p>
									)}
										...
										<ul className="space-y-2">
											{displayMyPostIds.map((postId) => (
												<li key={postId} data-testid="my-post-id-item">
													...
													{loadingMyPostIds.includes(postId) && (
														<p>読み込み中...</p>
													)}
													{myPostDetails[postId] && (
														<div>
															<p>本文: {myPostDetails[postId].body}</p>
															{typeof myPostDetails[postId].average_score ===
																"number" && (
																<p>{myPostDetails[postId].average_score}</p>
															)}
															{typeof myPostDetails[postId].rank ===
																"number" && (
