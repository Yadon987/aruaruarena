...
const MESSAGE_POST_DETAIL_RATE_LIMITED =
	"アクセスが集中しています。時間をおいて再度お試しください";
const MESSAGE_POST_DETAIL_SERVER_ERROR =
...
const MESSAGE_JUDGING_RETRY_GUIDE =
	"通信状況をご確認のうえ、再度お試しください。";
const MESSAGE_JUDGING_BACKEND_GUIDE =
	"backend を起動後に、再度お試しください。";
...
const MINIMUM_JUDGING_DURATION_STORAGE_KEY =
	"aruaruarena_minimum_judging_ends_at";
const JUDGING_POLLING_TIMEOUT_MS =
	import.meta.env.MODE === "development" && !isMockApiEnabled()
		? 120000
		: 60000;
...
const MESSAGE_REJUDGE_FAILED =
	"再審査に失敗しました。時間をおいて再度お試しください";
...
function isFinalResultPost(
	post: Post | null,
): post is Post & { status: "scored" | "failed" } {
	return (
		post !== null && (post.status === "scored" || post.status === "failed")
	);
}
...
function readFrontendBaseUrl(): string {
	...
	return envBaseUrl && envBaseUrl.length > 0
		? envBaseUrl
...
function canShowPostJudgingShareActions(
	...
	return (
		source === "judging" && isFinalResultPost(post) && post.status === "scored"
	);
}
...
function shouldOpenResultModalOnMyPostError(
...
function writePostIds(postIds: string[]) {
	localStorage.setItem(
		STORAGE_KEY,
		JSON.stringify(postIds.slice(0, MAX_STORED_POST_IDS)),
	);
}
...
function validateForm(nickname: string, body: string): ValidationErrors {
	...
	const nicknameError =
		nicknameLength < TEXT_LENGTH.NICKNAME_MIN ||
		nicknameLength > TEXT_LENGTH.NICKNAME_MAX
...
function isLocalWorkerUnavailable(health: GetHealthResponse): boolean {
	return (
		health.worker?.mode === "local_worker" &&
		health.worker.status === "unhealthy"
	);
}

async function resolveJudgingPollingErrorMessage(
...
function resolveJudgingSubmitErrorMessage(error: unknown): string {
	if (error instanceof ApiClientError) {
		...
		if (
			error.code === API_ERROR_CODE.TIMEOUT ||
			error.status === HTTP_STATUS.REQUEST_TIMEOUT
		) {
...
function App() {
	const soundControllerRef = useRef<ReturnType<
		typeof createSoundController
	> | null>(null);
	useAvatarImages();
	...
	const [myPostDetailErrors, setMyPostDetailErrors] = useState<
	...
	const [isStopJudgingConfirmOpen, setIsStopJudgingConfirmOpen] =
		useState(false);
	...
	const [resultModalErrorCode, setResultModalErrorCode] = useState<
		string | null
	>(null);
	const [resultViewSource, setResultViewSource] =
		useState<ResultViewSource>("unknown");
	...
	const [judgingTransientErrorCount, setJudgingTransientErrorCount] =
		useState(0);
	const [footerReservedSpace, setFooterReservedSpace] = useState(
	...
	const minimumJudgingTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	const minimumJudgingWaitResolverRef = useRef<
		((canProceed: boolean) => void) | null
	>(null);
	...
	useEffect(() => {
		...
		if (import.meta.env.MODE === "test" && !shouldShowOnboardingModalInTest())
			return;
	...
	const shareableResultPost = useMemo(() => {
		if (!canShowPostJudgingShareActions(activeResultPost, resultViewSource))
			return null;
	...
	const audioScene =
		resultAudioScene ?? (viewMode === "result" ? "top" : viewMode);
	...
	const fetchResultPost = useCallback(
	...
	const closeResultView = useCallback(() => {
		...
		requestAnimationFrame(() => {
			if (
				resultTriggerRef.current &&
				document.body.contains(resultTriggerRef.current)
			) {
	...
	const handleResultShareToX = useCallback(() => {
		...
		window.open(
			buildXShareIntentUrl(activeResultPost),
			"_blank",
			"noopener,noreferrer",
		);
	}, [activeResultPost, resultViewSource]);
		...
		const syncOgpStatus = async () => {
			...
			try {
				...
				queryClient.setQueryData(
					queryKeys.posts.detail(shareableResultPost.id),
					response,
				);
				setActiveResultPost(response);

				if (
					response.ogp_status === "ready" ||
					response.ogp_status === "failed"
				) {
					if (timerId) {
				...
				if (!isDisposed) {
					...
					if (
						consecutiveErrorCount >= RESULT_OGP_POLLING_MAX_ERRORS &&
						timerId
					) {
						clearInterval(timerId);
						timerId = null;
	...
	const enterJudgingMode = useCallback(
		(postId: string, isPollingReady: boolean = true) => {
	...
	const handleResultRejudge = useCallback(async () => {
		if (
			!activeResultPost ||
			activeResultPost.status !== "failed" ||
			isRejudging
		)
			return;
		...
		try {
			...
			const response = await api.posts.rejudge(
				activeResultPost.id,
				failedPersonas,
			);
			setIsRejudgeModalOpen(false);
	...
	useEffect(() => {
		...
		const handlePopState = () => {
			window.history.pushState(
				{},
				"",
				`${JUDGING_PATH_PREFIX}${judgingPostId}`,
			);
	...
	useEffect(() => {
		if (viewMode !== "judging" || !judgingPostId || !isJudgingPollingReady)
			return;

		let isDisposed = false;
		let isFailing = false;

		const handleJudgingFetchFailed = async (
			reason: "timeout" | "generic" = "generic",
		) => {
		...
		const fetchPost = async () => {
			...
			try {
				...
				if (
					error instanceof ApiClientError &&
					error.code === API_ERROR_CODE.ABORTED
				)
					return;
				...
				if (isTransientJudgingPollingError(error)) {
					...
					const transientElapsed =
						Date.now() - pollingTransientErrorStartedAtRef.current;
					if (
						pollingTransientErrorCountRef.current >=
							JUDGING_TRANSIENT_ERROR_MAX_RETRIES ||
	...
	const onSubmit = useCallback(
		async ({ nickname, body }: { nickname: string; body: string }) => {
			...
			const trimmedNickname = nickname.trim();
			const trimmedBody = body.trim();
			const { nicknameError: nextNicknameError, bodyError: nextBodyError } =
				validateForm(trimmedNickname, trimmedBody);
	...
	const fetchMyPostDetailForList = useCallback(
		async (postId: string, force: boolean = false) => {
			if (!force && myPostDetailsRef.current[postId])
				return myPostDetailsRef.current[postId];
	...
	const handleMyPostsTriggerKeyDown = (
	...
	const handlePostModalCloseWithDraft = useCallback(
		(draft: { nickname: string; body: string }) => {
	...
	useEffect(() => {
		...
		const updateFooterReservedSpace = () => {
			...
			const bottomOffset = Math.ceil(
				parseFloat(getComputedStyle(dockElement).bottom) || 0,
			);
	...
	return (
		<QueryClientProvider client={queryClient}>
			<AudioConsentModal
				isOpen={isAudioConsentModalOpen}
			...
			<div
				...
				style={{
					isolation: "isolate",
					paddingBottom: `${footerReservedSpace}px`,
				}}
			>
				{viewMode === "judging" && !judgingErrorMessage && <JudgingIndicator />}
				{viewMode === "judging" &&
					!judgingErrorMessage &&
					judgingTransientErrorCount > 0 && (
							...
							<motion.div
								...
								initial={
									prefersReducedMotion
										? {}
										: { opacity: 0, scale: SCALE.SHRUNK }
								}
								animate={
									prefersReducedMotion
										? {}
										: { opacity: 1, scale: SCALE.NORMAL }
								}
								...
								aria-label="自分の投稿"
								tabIndex={-1}
								className="modal-gorgeous-base relative z-10 flex max-h-full w-full max-w-[95vw] flex-col rounded-2xl p-4 text-slate-100 sm:max-w-2xl lg:max-w-3xl pointer-events-auto"
				...
				{viewMode !== "judging" && isFooterActionSheetOpen && (
					<div className="fixed inset-0 z-[60] flex items-center justify-center p-3">
						...
						<motion.div
							...
							initial={
								prefersReducedMotion ? {} : { opacity: 0, scale: SCALE.SHRUNK }
							}
							animate={
								prefersReducedMotion ? {} : { opacity: 1, scale: SCALE.NORMAL }
							}
							transition={{ duration: DURATION.MODAL }}
							role="dialog"
							aria-modal="true"
