...
import { AVATAR_ANIMATION, type AvatarState, getAvatarImagePath } from '../constants/avatar.ts'
...
export function useJudgeAvatar(persona: JudgePersona, isSpeaking: boolean): JudgeAvatarState {
  ...
  useEffect(() => {
    if (prefersReducedMotion) {
      setCurrentState((previousState) => (previousState === 'eye_closed' ? 'base' : previousState))
      return
    }
