	...
	describe("正常系: 型の整合性検証", () => {
		it("domain.ts で定数から型を導出している", () => {
			const domainContent = readFileSync(
				path.join(typesDir, "domain.ts"),
				"utf-8",
			);
			// 定数をインポートしていることを確認
			expect(domainContent).toMatch(
				/import\s+(type\s+)?\{\s*JUDGE,\s*POST_STATUS\s*\}\s+from/,
			);
			// 定数から型を導出していることを確認
		...
		it("domain.ts と constants/validation.ts で同じリテラル値を使用している", () => {
			...
			const constantsContent = readFileSync(
				path.join(constantsDir, "validation.ts"),
				"utf-8",
			);

			// 両方のファイルで同じ審査員ペルソナが使用されていることを確認
			expect(domainContent).toContain("'hiroyuki'");
			expect(constantsContent).toMatch(/["']hiroyuki["']/);
			expect(domainContent).toContain("'dewi'");
			expect(constantsContent).toMatch(/["']dewi["']/);
			expect(domainContent).toContain("'nakao'");
			expect(constantsContent).toMatch(/["']nakao["']/);
		});
	});
});
