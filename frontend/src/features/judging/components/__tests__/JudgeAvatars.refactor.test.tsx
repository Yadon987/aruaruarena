...
describe("JudgeAvatars Refactor", () => {
	...
	it("登場完了後は呼吸アニメーション設定を使う", async () => {
		...
		render(
			<JudgeAvatars viewMode="top" isJudging={true} isPostModalOpen={false} />,
		);

		expect(capturedImgProps.length).toBe(3);
		const nakao = capturedImgProps.find(
			(item) => item.alt === "中尾彬風審査員",
		);
	...
	it("発話中は対象審査員の吹き出しのみ表示する", async () => {
		const { JudgeAvatars } = await loadJudgeAvatars();
		render(
			<JudgeAvatars viewMode="judging" isJudging={true} isPostModalOpen={false} />,
		);
