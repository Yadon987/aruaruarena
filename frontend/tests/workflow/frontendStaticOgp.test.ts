...
const DEFAULT_OGP_IMAGE = resolve(ROOT_DIR, 'frontend/public/ogp/default.png')
...
describe('EP36-01: frontend static ogp', () => {
  it('index.html に静的 OGP と Twitter Card が設定されている', () => {
    ...
    expect(indexHtml).toContain('<meta property="og:image" content="%VITE_FRONTEND_BASE_URL%/ogp/default.png" />')
    ...
    expect(indexHtml).toContain('<meta name="twitter:image" content="%VITE_FRONTEND_BASE_URL%/ogp/default.png" />')
  })

  it('公開用デフォルト OGP 画像が frontend/public に存在する', () => {
    expect(existsSync(DEFAULT_OGP_IMAGE)).toBe(true)
  })
})
