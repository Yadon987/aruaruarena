...
| E14 | フロントエンド自動デプロイ（S3/CloudFront） | P1（高） | E02, E04 |  |
| E15 | 審査結果モーダル（フロントエンド） | P2（中） | E04, E07 |  |
| E16 | 自分の投稿一覧（フロントエンド） | P3（低） | E15 |  |
| E17 | プライバシーポリシー（フロントエンド） | P3（低） | E04 |  |
| E18 | BGM・SE再生 | P3（低） | E12, E13, E15 |  |
...
### E14: フロントエンド自動デプロイ（S3/CloudFront）

**概要**: `main` へのpushをトリガーに、フロントエンドを自動ビルドしてS3配信・CloudFrontキャッシュ無効化まで実行する

**ストーリー**:
- [ ] E14-01: フロントエンドデプロイ用GitHub Actionsワークフローの作成
- [ ] E14-02: `frontend` の本番ビルド（`npm ci && npm run build`）
- [ ] E14-03: S3への同期（`aws s3 sync`）
- [ ] E14-04: CloudFront invalidation の実行（`/*`）
- [ ] E14-05: `main` push時の自動実行条件と対象パス条件の整備
- [ ] E14-06: デプロイ失敗時の通知とロールバック手順のドキュメント化

**推奨Issue分割**:
- **Issue 1**: ワークフロー構築とSecrets整備 (E14-01, E14-05)
- **Issue 2**: S3同期とCloudFront invalidation (E14-03, E14-04, E14-06)

**受入基準**:
- `main` pushで自動実行される
- `frontend build → S3 sync → CloudFront invalidation` の順で処理される
- S3上の不要ファイルが削除され、配信物が最新化される
- CloudFront invalidation完了後に最新フロント画面を取得できる
- 失敗時にGitHub Actions上で原因を追跡できるログが残る
...
- [ ] E15-04: SNSシェアボタンの実装
- [ ] E15-05: OGP画像プレビュー
- [ ] E15-06: Framer Motionアニメーション
- [ ] E15-07: Playwright E2Eテスト

**推奨Issue分割**:
- **Issue 1**: モーダルUIと結果表示 (E15-01, E15-02, E15-06, E15-07の一部)
- **Issue 2**: アクションボタン機能（再審査・シェア） (E15-03, E15-04, E15-05, E15-07の一部)
...
**ストーリー**:
- [ ] E16-01: モーダルレイアウトの実装
- [ ] E16-02: LocalStorageからの取得
- [ ] E16-03: 投稿詳細への遷移
- [ ] E16-04: Playwright E2Eテスト
...
- [ ] E18-03: SE再生の実装
- [ ] E18-04: クロスフェード（0.5s）
- [ ] E18-05: ミュートトグル（LocalStorageに保存）
- [ ] E18-06: Playwright E2Eテスト
...
1. **E05: 投稿API**（Thread方式で非同期審査をトリガー）
2. **E06: AI審査システム**（3人並列実行）
3. **E07: 投稿詳細API**（審査結果結合）


### フェーズ2: ランキング・公開

1. **E08: ランキングAPI**（TOP20取得）
2. **E09: レート制限・スパム対策**（5分制限・重複チェック）
3. **E10: OGP画像生成**（SNSシェア用）


### フェーズ3: 再審査・高度な機能

1. **E11: 再審査API**（失敗審査員のみ再実行）


### フェーズ4: フロントエンド実装

1. **E12: トップ画面**（モック使用で先行開発可）
2. **E13: 審査中画面**（モック使用で先行開発可）
3. **E15: 審査結果モーダル**（モック使用で先行開発可）
...
1. **E14: フロントエンド自動デプロイ**（main pushで build → S3 sync → CloudFront invalidation）
...
| 日付 | バージョン | 内容 |
|------|-----------|------|
| 2026-02-17 | 2.2 | E18をE14へ再採番し、E14以降のEpicとストーリーIDを連番に再編成 |
| 2026-02-17 | 2.1 | E18（フロントエンド自動デプロイ）を追加し、main push時の build → S3 sync → CloudFront invalidation を明文化 |
