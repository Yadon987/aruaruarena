...
### データの取り扱い
- **投稿データ**: ニックネーム、投稿本文、採点結果、順位は本サービス上で公開されることがあります。
- **AI審査**: 投稿本文は Gemini / Cerebras / Groq 等の外部AI APIへ送信されます。
- **不正利用対策**: IPアドレス・ニックネームのハッシュ化レート制限、本文ハッシュによる24時間の重複検知を行います。
- **ブラウザ保存**: `localStorage` に自分の投稿ID、音量設定、音声モーダル回答状態を保存します。
- **アクセスログ**: CloudFront / S3 / アプリケーションログを運用・障害調査のため保存します。
