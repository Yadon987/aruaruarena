...
【プラン or issue 参照内容】
- `.github/E15-01_RESULT_MODAL_UI_SPEC.md`
...
- [重要度: 高]
- 問題点: `status=scored` の場合に `rank` / `total_count` が欠損したときの扱いが「フォールバック表示」としか書かれておらず、UI仕様が未確定。実装者ごとに表示が分岐し、受入判定不能になる。
- 改善提案: `status=scored` で `rank` または `total_count` 欠損時は「順位情報を取得できませんでした」を固定表示し、`failed` 時の `---` と区別する。

- [重要度: 中]
- 問題点: モーダルのアクセシビリティ要件にフォーカストラップとフォーカス復帰が未定義。キーボード操作時に背景へフォーカスが抜けるリスクがある。
- 改善提案: 初期フォーカス、タブ循環、クローズ時のトリガー要素へのフォーカス復帰を必須要件として明記する。

- [重要度: 中]
- 問題点: データ取得タイミングが曖昧で、モーダルオープン時に毎回取得するのか、事前取得済みデータを使うのか不明。二重リクエストや表示チラつきの原因になる。
- 改善提案: `usePost(id)` を利用し、`id` 変更時のみ取得、同一 `id` 再オープン時はキャッシュ表示を優先する方針を明記する。

- [重要度: 中]
- 問題点: エラー時の表示要件はあるが、`NOT_FOUND` と一時エラー（5xx/ネットワーク）のユーザー向け文言・挙動差が定義されていない。
- 改善提案: `NOT_FOUND` は「投稿が見つかりません」、それ以外は「投稿詳細の取得に失敗しました」として再試行導線を用意する。

- [重要度: 低]
- 問題点: パフォーマンス要件「300ms以内」が測定条件未定義で検証不能。
- 改善提案: ローカル開発環境（`npm run dev`、キャッシュヒット時）を対象条件として明記する。

- [重要度: 低]
- 問題点: テスト観点に「連続クリック」「連続開閉」の並行操作が不足。
- 改善提案: 同一投稿の連続オープンで重複フェッチしないこと、異なる投稿IDへの切り替えで前リクエスト結果が後勝ちしないことを追加する。
...
上書き対象: `.github/E15-01_RESULT_MODAL_UI_SPEC.md`
